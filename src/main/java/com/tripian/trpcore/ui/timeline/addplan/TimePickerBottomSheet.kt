...
class TimePickerBottomSheet : BaseSimpleBottomSheet<BottomSheetTimePickerBinding>(
    ...
    /** Selected times in "HH:mm" (24h). */
    private var selectedStartTime: String? = null
    private var selectedEndTime: String? = null

    /** True if endTime was already set when the sheet opened; disables end-time auto-calculation. */
    private var wasEndTimeInitiallySet: Boolean = false

    /** Duration in minutes for auto-calculating end time. */
    private var durationMinutes: Int = DEFAULT_DURATION_MINUTES

    /** Completely disables auto end time calculation (for smart recommendations). */
    ...
    private fun updateDisplayTexts() {
        ...
        binding.tvEndTimeDisplay.setTextColor(
            requireContext().getColor(
                when {
                    isEndTimeInvalid -> R.color.trp_error_message
