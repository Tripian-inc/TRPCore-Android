...
class FRPastTripVM @Inject constructor(
    val getUserTrip: GetUserTrip,
    ...
    fun onClickedMore(trip: Trip) {
        navigateToFragment(fragment = FRMoreSelection.newInstance(trip, false))
    }
    ...
    private fun update(useCache: Boolean = false) {
        ...
        getUserTrip.on(GetUserTrip.Params(isUpComing = false, useCache = useCache), success = {
